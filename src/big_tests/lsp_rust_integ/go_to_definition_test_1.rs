...
fn go_to_definition_test_1() {
    ...
    assert!(full_setup.wait_for(|f| {
        f.get_first_editor().unwrap().get_visible_cursor_cells().next().is_some()
    }));
}

#[test]
fn go_to_definition_test_2() {
    let mut full_setup: FullSetup = FullSetup::new("./test_envs/big_test_env")
        .with_files(["src/reader/reader_main_widget.rs"])
        .with_mock_navcomp(false)
        .build();
        ...
        assert!(full_setup.wait_for(|full_setup| {
            full_setup
                ...
                .unwrap()
                .get_visible_cursor_cells()
                .find(|(pos, cell)| cell.grapheme() == Some("t"))
                .is_some()
        }));
