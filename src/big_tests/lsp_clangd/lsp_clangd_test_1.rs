...
fn completions_clangd_integ_test_1() {
    ...
    assert!(full_setup.wait_for(|f| {
        f.get_first_editor().unwrap().get_visible_cursor_lines().find(|line| line.contents.text.trim() == "some.⏎").is_some()
    }));

    assert!(full_setup.send_key(Keycode::Space.to_key().with_ctrl()));

    assert!(full_setup.wait_for(|f| {
        f.get_first_editor().unwrap().completions().map(|comp| comp.is_loading()) == Some(false)
    }));

    let items: Vec<String> = full_setup.get_first_editor().unwrap().completions().unwrap().items().map(|item| {
        item.text
    }).collect();

    // each infix should appear at least once
    let expected_infixes: Vec<&'static str> = vec![
        "assign(",
        "at(size_type n)",
        "back()",
        "begin()",
    ...
    assert!(full_setup.get_first_editor().unwrap().completions().unwrap().highlighted(true).unwrap().1.contains(expected_highlight));

    for infix in expected_infixes.iter() {
        assert!(full_setup.get_first_editor().unwrap().completions().unwrap().items().find(|item| item.text.contains(infix)).is_some());
    }
