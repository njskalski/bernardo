...
fn show_usages_test_2_escape_closes() {
    ...
    assert!(full_setup.send_key(full_setup.config().keyboard_config.global.close_context_menu));

    assert!(full_setup.wait_for(|f| { f.get_first_editor().unwrap().context_bar_op().is_none() }));
}
