...
impl NavCompLoader for RealNavCompLoader {
    ...
    fn load_handler(&self,
                    ...
                    navcomp_tick_sender: NavCompTickSender) -> Result<Box<dyn Handler>, HandlerLoadError> {
        match &project_scope.handler_id {
            None => {
                warn!("project scope [{:?}] with no handler - what the point?", project_scope.path.relative_path());
                Err(HandlerLoadError::NoHandlerId)
            }
            Some(handler_id) => {
                Ok(handler_factory(config,
                                   &handler_id,
